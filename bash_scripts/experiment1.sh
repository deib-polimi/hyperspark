...
for i in "${instances[@]}"; do							#for each instance
	for c in "${cores[@]}"; do							#for each number of cores to use
		for run in $(seq 1 10); do						#run 10 times
			...
			app_id=`grep 'tracking' <<< "$(exec spark-submit --conf spark.root.logger=INFO,console --num-executors $c --class it.polimi.hyperh.experiments.Experiment1 hyperh-0.0.1-SNAPSHOT.jar $i $c   2>&1)" | head -n1 | cut -c57-86`
			echo $app_id
			...
			echo "$log_contents" | sudo tee -a experiment1.txt
		done
	done
done
